...
*   **Benefit**: If an Out-Of-Memory (OOM) error occurs in the Worker Process, it can be safely terminated and restarted without affecting the main HTTP server, ensuring service availability.
...
2.  **vLLM** (`engine: vllm`):
    ...
    *   Supports native parallel requests, AWQ/GPTQ/F8 quantization.
