...
Windows-specific launcher: opens one Windows terminal window per prompt and
runs each chat synchronously inside its own worker process.

This script opens one Windows terminal window per prompt and runs each chat
synchronously inside its own worker process.

Usage:
    uv run openai-compatible-parallel-terminal-batch-example.py
    uv run openai-compatible-parallel-terminal-batch-example.py --settings app_settings/app_settings_parallel_terminal_batch_chat.json
...
def spawn_worker_windows(
    ...
    """Spawn one Windows console window per prompt."""
    if os.name != "nt":
        raise RuntimeError(
            "This script requires Windows to open separate terminal windows."
        )
...
def run_controller(settings_path: str) -> None:
    ...
    processes = spawn_worker_windows(settings_path, len(prompts))
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Open N terminal windows and run one synchronous chat per window.",
    )
    parser.add_argument(
        "--settings",
        type=str,
